...
**Medical Symptom-to-Disease Classification | Python, PyTorch, Hugging Face Transformers**

- Built an end-to-end NLP pipeline by consolidating and cleaning two symptom datasets into 1,988 unique descriptions spanning 24 disease classes, with stratified 70/15/15 train, validation, and test splits.
- Benchmarked BiLSTM with FastText against DistilBERT, Bio_ClinicalBERT, and BiomedBERT; achieved **99.00% test accuracy** and **99.04% macro F1** with Bio_ClinicalBERT on 299 held-out examples.
- Applied class-weighted training, dropout, early stopping, per-class evaluation, confusion matrices, and error analysis, reducing test errors from 15 for the BiLSTM baseline to 3 for the best domain-specific transformer.
